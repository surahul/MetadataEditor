...
public class FlacInfoReader {
    ...
    public FlacAudioHeader read(File file) throws CannotReadException, IOException {
        logger.config(file.getAbsolutePath() + ":start");
        ...
        while (isLastBlock == false) {
            ...
            logger.info(file.getAbsolutePath() + " " + mbh.toString());
            if (mbh.getBlockType() == BlockType.STREAMINFO) {
                mbdsi = new MetadataBlockDataStreamInfo(mbh, fc);
                if (!mbdsi.isValid()) {
                    throw new CannotReadException(file.getAbsolutePath() + ":FLAC StreamInfo not valid");
                }
            } else {
        ...
        if (mbdsi == null) {
            throw new CannotReadException(file.getAbsolutePath() + ":Unable to find Flac StreamInfo");
        }
