...
public class FlacTagWriter {
    ...
    public void write(Tag tag, File file) throws CannotWriteException {
        logger.config(file.getAbsolutePath() + " Writing tag");
        try {
            ...
            logger.config(file.getAbsolutePath() + ":Writing tag available bytes:" + availableRoom + ":needed bytes:" + neededRoom);
            ...
            if ((availableRoom == neededRoom) || (availableRoom > neededRoom + MetadataBlockHeader.HEADER_LENGTH)) {
                logger.config(file.getAbsolutePath() + ":Room to Rewrite");
            ...
            //Need to move audio
                ...
                logger.config(file.getAbsolutePath() + ":Audio must be shifted " + "NewTagSize:" + newTagSize + ":AvailableRoom:" + availableRoom + ":MinimumAdditionalRoomRequired:" + (neededRoom - availableRoom));
    ...
    private void insertUsingChunks(File file, Tag tag, FileChannel fc, MetadataBlockInfo blockInfo, FlacStreamReader flacStream, int neededRoom, int availableRoom) throws IOException, UnsupportedEncodingException {
        ...
        logger.config(file.getAbsolutePath() + " Audio needs shifting:" + extraSpaceRequired);
