...
public class DsfFileWriter extends AudioFileWriter2 {
    protected void writeTag(Tag tag, File file) throws CannotWriteException {
            ...
            if (dsd != null) {
                if (dsd.getMetadataOffset() > 0) {
                    ...
                    if (id3Chunk != null) {
                        ...
                        throw new CannotWriteException(file.getAbsolutePath() + "Could not find existing ID3v2 Tag");
                    }
                } else {
        ...
        } catch (CannotWriteException ade) {
            throw new NoWritePermissionsException(file.getAbsolutePath() + ":" + ade.getMessage());
