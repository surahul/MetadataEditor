...
class MediaStoreHack {
     ...
     */
    boolean delete(final Context context, final File file) {
        try {
    ...
    private File getExternalFilesDir(final Context context) {
    ...
    FileOutputStream getOutputStream(Context context, String str, boolean append) {
        FileOutputStream outputStream = null;
        Uri fileUri = getUriFromFile(str, context);
        if (fileUri != null) {
            try {
                outputStream = (FileOutputStream)( append ? context.getContentResolver().openOutputStream(fileUri, "rw") : context.getContentResolver().openOutputStream(fileUri));
            } catch (Throwable th) {
            }
        }
        return outputStream;
    ...
    Uri getUriFromFile(final String path, Context context) {
        try {
    ...
    private int getTemporaryAlbumId(final Context context) {
    ...
    private
    @Nullable
    File installTemporaryTrack(final Context context) throws IOException {
    ...
    boolean mkfile(final Context context, final File file) {
        final OutputStream outputStream = getOutputStream(context, file.getPath(), true);
